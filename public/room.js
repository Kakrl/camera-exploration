    ...
    stats.forEach(report => {
      if ((report.type === 'inbound-rtp' || report.type === 'outbound-rtp') && report.kind === 'video') {
        if (lastStats) {
          ...
          if (prev) {
            const deltaT = (report.timestamp - prev.timestamp) / 1000;
            ...
            const fps = (frames - prevFrames) / deltaT;
            
            if (!isNaN(fps)) document.getElementById('val-fps').innerText = Math.round(fps);
...
async function setFrameRateMode(fpsValue) {
  ...
  const val = parseInt(fpsValue);

  const constraints = {
    frameRate: { ideal: val, max: val } 
  };

  try {
    await videoTrack.applyConstraints(constraints);
    console.log(`Frame rate set to: ${val} FPS`);
  } catch (e) {
